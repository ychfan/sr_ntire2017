...
def dataset(hr_flist, lr_flist, resize_func):
    ...
    hr_patches0, lr_patches0 = make_patches(hr_image, lr_image, resize_func)
    hr_patches1, lr_patches1 = make_patches(tf.image.rot90(hr_image), tf.image.rot90(lr_image), resize_func)
    return tf.concat([hr_patches0, hr_patches1], 0), tf.concat([lr_patches0, lr_patches1], 0)

def make_patches(hr_image, lr_image, resize_func):
